...
public class LoggerConstants {
  ...
  public static void setLogData(HttpServletRequest request, HttpServletResponse response) {
    ...
    String userId =
        StringUtils.isEmpty(xMasheryToken)
            ? ""
            : MasheryObject.getInstance(xMasheryToken).getCcoId();
    ...
    if (!StringUtils.isEmpty(puId)) {
      MDC.put(LoggerConstants.PUID, puId);
    }
    if (!StringUtils.isEmpty(customerIdReqParam)) {
      MDC.put(LoggerConstants.CUSTOMER_ID, customerIdReqParam);
    }
    ...
    while (headerNames.hasMoreElements()) {
      ...
      if (key.trim().equalsIgnoreCase(LoggerConstants.AUTHORIZATION)) {
        continue;
      }
      if (key.trim().equalsIgnoreCase(LoggerConstants.X_REQUEST_ID)) {
        continue;
      }
    ...
    if (request.getHeader(LoggerConstants.X_ORIGINAL_FORWARDED_FOR) != null) {
      MDC.put(
          LoggerConstants.CLIENT_IP, request.getHeader(LoggerConstants.X_ORIGINAL_FORWARDED_FOR));
    }
    ...
    LOG.info(
        ...
        request.getMethod(),
        MDC.get(LoggerConstants.REQUEST_URI));
