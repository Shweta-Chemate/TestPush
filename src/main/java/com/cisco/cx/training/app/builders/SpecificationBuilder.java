...
public class SpecificationBuilder {
      ...
      if (key.equals(Constants.CONTENT_TYPE_FIELD)) {
        ...
        for (String contentType : values) {
          contentTypeSpecification =
              contentTypeSpecification.or(CustomSpecifications.findWithCriteria(key, contentType));
        }
      ...
      for (String searchField : getSearchFields()) {
        searchSpecification =
            searchSpecification.or(
                CustomSpecifications.searchItemsWithCriteria(searchField, search));
      }
  ...
  public <T> Specification<T> filterById(List<String> learningItemIdsListForYou) {
    ...
    if (!learningItemIdsListForYou.isEmpty()) {
      ...
      specification =
          specification.and(
              CustomSpecifications.hasValueIn(Constants.ID, learningItemIdsListForYou));
    }
    return specification;
  }
