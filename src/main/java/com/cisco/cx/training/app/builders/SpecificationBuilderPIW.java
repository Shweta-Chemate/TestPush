...
public class SpecificationBuilderPIW {
	...
	public <T> Specification<T> filter(Map<String, String> queryFilters, String search, String region) {
		Specification<T> specification = Specification.where(null);

		//filter piws learning type
		specification= specification.and(CustomSpecifications.hasValue(Constants.LEARNING_TYPE, Constants.PIW));

		if(region!=null)
		{
			specification = specification
					.and(CustomSpecifications.hasPIWs(Constants.REGION, region));
		}
		if (search != null) {
			try {
	...
	public <T> Specification<T> buildFilterSpecification(Map<String, String> filterParams) {
		...
		for (Map.Entry<String, String> filterParam : filterParams.entrySet()) {
			...
			if (key.equalsIgnoreCase(Constants.MOST_POPULAR)) {
				specification = specification.and(CustomSpecifications.hasGreaterThanMinimumScore(Constants.SCORE_COLUMN));	
			}

			if (key.equalsIgnoreCase(Constants.BOOKMARKED) || key.equalsIgnoreCase(Constants.REGISTERED)) {
