...
public class TPService extends ProductDocumentationService {
  ...
  private void initializeTPFiltersByCards(
    ...
    countFilters.put(CONTENT_TYPE_FILTER_TP, allContentsCT);
    ...
    countFilters.put(TECHNOLOGY_FILTER_TP, allContentsTC);
    ...
    countFilters.put(LANGUAGE_FILTER_TP, allContentsLG);
    ...
    countFilters.put(LIVE_EVENTS_FILTER_TP, allContentsLE);
    ...
    List<Map<String, Object>> dbListST =
        productDocumentationDAO.getAllStUcWithCountByCards(
            contentTab,
            cardIds,
            hcaasStatus); // productDocumentationDAO.getAllStUcPsWithCount(contentTab);
    Map<String, Object> allContentsST = ProductDocumentationUtil.listToSTMap(dbListST, stFilter);
    countFilters.put(SUCCESS_TRACKS_FILTER_TP, allContentsST);
    ...
    countFilters.put(LIFECYCLE_FILTER_TP, allContentsLC);
    ...
    countFilters.put(ROLE_FILTER_TP, allContentsRole);
  ...
  public Map<String, Object> getTopPicksFiltersViewMore(
    ...
    initializeTPFiltersByCards(
        filters, countFilters, contentTab, hcaasStatus, cardIds); // extra but must here
  ...
  /** {"Language":["English","Japanese"], "Toppicks":["CI-100","SWAP-200"]} * */
  public LearningRecordsAndFiltersModel getTopPicksCardsViewMore(
      String userId, Map<String, Object> applyFilters, boolean hcaasStatusFlag) {

    /** common start * */
    String hcaasStatus = String.valueOf(hcaasStatusFlag);
    LOG.info("tpc hcaas{}", hcaasStatus);
    String contentTab = Constants.TOPPICKS;
    ...
    if (!filteredTPCards.isEmpty() && !CollectionUtils.isEmpty(applyFilters)) {
      Map<String, Set<String>> filteredCardsMap =
          filterTpCards(applyFilters, contentTab, hcaasStatus, filteredTPCards);
    ...
    if (!filteredTPCards.isEmpty()) {
      ...
      dbCards.addAll(
          productDocumentationDAO.getAllLearningCardsByFilter(
              contentTab,
              filteredTPCards,
              Sort.by(DEFAULT_SORT_ORDER, DEFAULT_SORT_FIELD),
              hcaasStatus));
  ...
  private Map<String, Set<String>> filterTpCards(
      Map<String, Object> applyFilters,
      String contentTab,
      String hcaasStatus,
      Set<String> tpCardIds) {
        ...
        .forEach(
            k -> {
              ...
              if (v instanceof List) {
                list = (List<String>) v;
                switch (k) {
                  case TECHNOLOGY_FILTER_TP:
                    filteredCards.put(
                        k,
                        productDocumentationDAO.getTpCardIdsByTC(
                            contentTab, new HashSet<>(list), hcaasStatus, tpCardIds));
                    break;
                  case LIVE_EVENTS_FILTER_TP:
                    filteredCards.put(
                        k,
                        productDocumentationDAO.getTpCardIdsByRegion(
                            contentTab, new HashSet<>(list), hcaasStatus, tpCardIds));
                    break;
                  case CONTENT_TYPE_FILTER_TP:
                    filteredCards.put(
                        k,
                        productDocumentationDAO.getTpLearningsByContentType(
                            contentTab, new HashSet<>(list), hcaasStatus, tpCardIds));
                    break;
                  case LANGUAGE_FILTER_TP:
                    filteredCards.put(
                        k,
                        productDocumentationDAO.getTpCardIdsByLanguage(
                            contentTab, new HashSet<>(list), hcaasStatus, tpCardIds));
                    break;
                  case ROLE_FILTER_TP:
                    filteredCards.put(
                        k,
                        productDocumentationDAO.getTpCardIdsByRole(
                            contentTab, new HashSet<>(list), hcaasStatus, tpCardIds));
                    break;
                  case LIFECYCLE_FILTER_TP:
                    filteredCards.put(
                        k,
                        productDocumentationDAO.getTpCardIdsByPs(
                            contentTab, new HashSet<>(list), hcaasStatus, tpCardIds));
                    break;
                    // c+ productDocumentationDAO.getTpCardIdsByCiscoPlus(contentTab,new
                    // HashSet<>(list),hcaasStatus, tpCardIds));break;
                  default:
                    LOG.info("TP other {}={}", k, list);
                }
                ;
                    ...
                    .forEach(
                        ikTp -> {
                          ...
                          if (ivTp instanceof Map) {
                            Set<String> usecasesTp = ((Map) ivTp).keySet();
                            String successtrackTP = ikTp.toString();
                            cardIdsStUcPs.addAll(
                                productDocumentationDAO.getTpCardIdsByPsUcSt(
                                    contentTab,
                                    successtrackTP,
                                    usecasesTp,
                                    hcaasStatus,
