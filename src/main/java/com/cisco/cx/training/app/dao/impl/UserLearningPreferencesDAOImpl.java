...
public class UserLearningPreferencesDAOImpl implements UserLearningPreferencesDAO {
  ...
  @Autowired
  public UserLearningPreferencesDAOImpl(
  ...
  @Override
  public Map<String, List<UserLearningPreference>> createOrUpdateULP(
    ...
    if (currentULPs == null || currentULPs.isEmpty()) {
      ...
      Arrays.asList(PREFERENCES_KEYS.values())
          .forEach(
              preferenceKey -> {
                if (ulPreferences.containsKey(preferenceKey.name())) {
                  ...
                  if (!preferenceNames.isEmpty()) {
                    itemValue.put(
                        preferenceKey.name().concat(PREFERENCE_SUFFIX),
                        AttributeValue.builder().ss(preferenceNames).build());
                  }
                }
              });
    }
  ...
  public QueryResponse fetchULPPreferencesDDB(String userId) {
    ...
    expressionAttributeValues.put(
        ":useridValue", AttributeValue.builder().s(userId.concat(USERID_SUFFIX)).build());
  ...
  public Map<String, List<UserLearningPreference>> fetchUserLearningPreferences(String userId) {
    ...
    String hcaasStatus =
        String.valueOf(
            partnerProfileService.getHcaasStatusForPartner(
                (String)
                    request
                        .getServletContext()
                        .getAttribute(Constants.MASHERY_HANDSHAKE_HEADER_NAME)));
                    ...
                    ulps.forEach(
                        timeInterval -> {
                          try {
                            ...
                            LOG.warn(
                                "Invalid TimeInterval preference {} err={}",
                                timeInterval,
                                e.getMessage());
                          }
                        });
  ...
  private Map<? extends String, ? extends List<UserLearningPreference>>
