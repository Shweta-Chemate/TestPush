...
public class SpecificationBuilderSuccessTalk {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(SpecificationBuilderSuccessTalk.class);
  ...
  public <T> Specification<T> filter(Map<String, String> queryFilters, String search) {
    ...
    specification =
        specification.and(
            CustomSpecifications.hasValue(Constants.LEARNING_TYPE, Constants.SUCCESSTALK));
      ...
      for (String searchField : getSearchFields()) {
        searchSpecification =
            searchSpecification.or(
                CustomSpecifications.searchSuccesstalksWithCriteria(searchField, search));
      }
