...
@ContextConfiguration(classes = { TrainingAndEnablementApplication.class, PropertyConfiguration.class,
		ElasticSearchConfig.class, Swagger2Config.class, FilterConfig.class })
...
public class TrainingAndEnablementControllerTest {
	...
	/*
	 * @Test public void testFetchSuccessTalks() throws Exception { this.mockMvc
	 * .perform(get("/v1/partner/training/successTalks").contentType(MediaType.
	 * APPLICATION_JSON_VALUE) .header("X-Mashery-Handshake",
	 * this.XMasheryHeader).characterEncoding("utf-8"))
	 * .andDo(print()).andExpect(status().isOk()); }
	 */
	...
	public void getUserSuccessTalks() throws Exception {
		this.mockMvc
				.perform(get("/v1/partner/training/successTalks/email").contentType(MediaType.APPLICATION_JSON_VALUE)
						.header("X-Mashery-Handshake", this.XMasheryHeader).characterEncoding("utf-8"))
				.andDo(print()).andExpect(status().isOk());
	}
	...
	public void createorUpdateBookmark() throws Exception {
		...
		this.mockMvc
				.perform(post("/v1/partner/training/successTalk/bookmarks")
